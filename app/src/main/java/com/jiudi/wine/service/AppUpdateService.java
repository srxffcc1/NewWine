package com.jiudi.wine.service;
...
import com.jiudi.wine.BuildConfig;
import com.jiudi.wine.R;
import com.jiudi.wine.util.LogUtil;
...
public class AppUpdateService extends Service {
                ...
                public void onProgress(int progress, long networkSpeed) {
                    ...
                    if (mOldProgress == 0 || (mOldProgress > 0 && (progress - mOldProgress > 10) || progress == 100)) {
                        if (mOnDownloadProgressChangeListener != null) {
                            mOnDownloadProgressChangeListener.onProgressChange(progress);
                        }
                        notifyUser(progress);
    ...
    public static interface OnDownloadProgressChangeListener {
        void onProgressChange(int progress);
    }


    private static Intent intent;
    private static OnDownloadProgressChangeListener mOnDownloadProgressChangeListener;
     ...
     */
    public static void start(Context context, String savePath, String downloadUrl, OnDownloadProgressChangeListener listener) {
        intent = new Intent(context, AppUpdateService.class);
        intent.putExtra("save_path", savePath);
        intent.putExtra("download_url", downloadUrl);
        mOnDownloadProgressChangeListener = listener;
        context.startService(intent);
    }

    public static void stop(Activity activity) {
        if (intent != null && !TextUtils.isEmpty(mDownloadUrl)) {
            LogUtil.e(TAG,"stop");
            HttpRequest.cancel(mDownloadUrl);
